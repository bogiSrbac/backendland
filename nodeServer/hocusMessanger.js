...
var socket = io('http://127.0.0.1:666/');

socket.on('provjera veze1', msg=>{
    console.log(msg);
})
...
// socket.on('leave', msg=>{
//     console.log('Player ', msg, ' leave room!')
// })
...
amqp.connect('amqp://bogi:bogi@rabbitmq:5672',  function(error0, connection) {
  ...
  connection.createChannel(function(error1, channel) {
    ...
    channel.consume(queue, function reply(msg) {
      ...
      //creating object with data players in room hocus
      ...
      setTimeout(function () {
      //   setRoomData(listOfHocusPlayersId, listOfPlayers1, listOfNicknames1, listOfIDs);
      //   console.log(listOfHocusPlayersId);
    ...
    channel.assertQueue('', {
      ...
      channel.consume(q.queue, function(msg) {
        if(msg.content) {
            ...
            console.log(nick)
            setTimeout(function () {
                    if(typeof nick.nickName !== 'undefined'){
               socket.emit('nick', nick);
               socket.on('idToList', msg=>{
        })
            }else if(nick.shieldPoints){
                        socket.emit('shieldResponse', nick)
                    }
                    else if (nick.attackPoints){
                        socket.emit('attackResponse1', nick);

                    }else if(nick.hocusHealth){
                        socket.emit('hocusHealthUpdate', nick)
                    }else if(nick.win){
                        socket.emit('win', nick)
                    }else if(nick.winEnd){
                        socket.emit('win', nick)
                    }
                    if(nick.PocusAttackToHocus === 'attack'){
